...
function newCounter(num:number){
    let count = num;
    return  {
        increase(){
            return count+=1;
...
//@ts-ignore
function flatten(arrs:any){
    return arrs.reduce((acc:any[], cur:any[]) => acc.concat(Array.isArray(cur) ? flatten(cur) : cur), []);
}
console.log("Task 06")
console.log(flatten([[1, 2], [3, [1, [11]], 4]]));
